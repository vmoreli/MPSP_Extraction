...
def run_all_prompts_once(doc_id: str, raw_text: str) -> Dict[str, Any]:
    ...
    # 2) inquerito (precisa do enum do mapeamento -> passa como texto)
    ...
    # 8) tudo em um (seu experimento original conta 8 “sem retries”; aqui você escolhe.
...
def save_graphs(doc_id: str, cache: Dict[str, Any]) -> None:
    ...
    for variant_name, payload in variants.items():
        out_path = Path("results_variants") / "graphs" / variant_name / f"{doc_id}.json"
...
def main():
    if len(sys.argv) < 2:
        raise SystemExit("Uso: python -m graph_variantes.run_variants <caminho_arquivo.txt>")
    ...
    # =========================================================
    PROJECT_ROOT = Path(__file__).resolve().parents[1]  # MPSP_Extraction-main
    GRAPHS_DIR = PROJECT_ROOT / "results_variants" / "graphs"
