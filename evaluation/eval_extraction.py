...
def main():
    ...
    PRED_FILEPATH = "extraction_results/st_5_gemini-2.5-pro_results.json"
    ...
    for pid, gt_proc in gt_data.items():
        ...
        try:
            ...
            # FIX: pipeline salva como 'vitimas' (sem acento); junta.py salva como 'vítimas' (com acento).
            # Tentamos os dois para suportar resultados de ambas as origens.
            _v = pred_proc.get('vítimas') or pred_proc.get('vitimas') or {}
            pred_vits = _v.get('vitimas', []) if isinstance(_v, dict) else (_v or [])
            eval_list_of_objects(gt_vits, pred_vits, eval_vitima, "vítimas", pid, collector)

            gt_susp = gt_proc.get('suspeitos', [])
            # FIX: schema define o campo como 'Suspeitos' (maiúsculo); busca com a grafia correta.
            _s = pred_proc.get('suspeitos') or {}
            pred_susp = _s.get('Suspeitos', []) if isinstance(_s, dict) else (_s or [])
